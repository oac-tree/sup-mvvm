...
TEST_F(NotifyingModelTests, InsertItemIntoParentUsingTagAndIndexViaGetModel)
{
  auto parent = m_model.InsertItem<CompoundItem>();
  parent->RegisterTag(TagInfo::CreateUniversalTag("tag"), false);
...
TEST_F(NotifyingModelTests, InsertItemInPropertyTag)
{
  ...
  EXPECT_THROW(m_model.InsertItem<PropertyItem>(parent, {"thickness", 0}), InvalidOperationException);
